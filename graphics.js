...
// отрисовать график по набору точек
	drawGraphic(xArr, yArr) {
		const xExtrems = xArr.getExtrems()
		const yExtrems = yArr.getExtrems()
		
		const xMin = MathExtended.getOrder(xExtrems.min)
		const xMax = MathExtended.getOrder(xExtrems.max)
		
		const yMin = MathExtended.getOrder(yExtrems.min)
		const yMax = MathExtended.getOrder(yExtrems.max)
